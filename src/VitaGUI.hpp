...
class VitaGUI{
	public:
		...
		int click(int x , int y, uint64_t duration = 0);
		...
		void DrawTextWithEmojis(std::string text, int startX, int startY, int size, int maxWidth = 900, std::vector<parsed_url>* urls = nullptr);
	...
	private:
		...
		int state = 0;  // 0 login , 1 loading , 2 guilds , 3 channels , 4 channellobby , 6 dms , 7 dmchannellobby , 15 settings , 85818764 stuff..
